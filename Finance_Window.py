...
def FinancesDisplay(main_window):
    ...
    welcome_message = Text(window2, text="Finance Statistics" , size=15, font="Times New Roman",grid=[0,0,4,1])
    listbox = ListBox(window2, items=[],multiselect=True,width=400,height=200,scrollbar=True,grid=[0,1,4,5])

    update_listbox()

    #options
    new_expence = PushButton(window2,text='Create New Expence',command=create_expence,grid=[0,7,1,1])

    rebuild = PushButton(window2,text='Rebuild Statistics',command=update_listbox,grid=[1,8,1,1])
    pricing_button = PushButton(window2,text='Update Pricing',command=SyncSheetItems,grid=[0,8,1,1])
