...
def GeneratePackingSlip(order):
    ...
    modd.text((20, 730), order.getOrderAddress()[
              0], font=normal, fill=(text_color))
    modd.text((20, 770), order.getOrderAddress()[2] + ', ' + order.getOrderAddress()[
              3] + ' ' + order.getOrderAddress()[4], font=normal, fill=(text_color))
    ...
    modd.text((20, 1000), 'Waterford, Pa 16441',
              font=normal, fill=(text_color))

    # Order Number (100 between)
    modd.text((20, 1100), 'Order Number:', font=normal, fill=(text_color))
    modd.text((20, 1150), str(order.getOrderNumber()),
              font=normal, fill=(text_color))
    ...
    # Quantity
    modd.text((500, 650), str(len(order.getOrderItems())) +
              ' Items', font=normal, fill=(text_color))
    ...
    for i in range(len(order.getOrderItems())):
        if resize == False:
            modd.text((520, 720+(i+1)*50), str(order.getOrderItems()
                      [i].getQuantity()), font=normal, fill=(text_color))
            modd.text((650, 720+(i+1)*50), str(order.getOrderItems()
                      [i].getProduct()), font=normal, fill=(text_color))
            modd.text((1050, 720+(i+1)*50), '$' + str(order.getOrderItems()
                      [i].getUnitPrice(order.getPricingStyle())/100), font=normal, fill=(text_color))
            modd.text((1200, 720+(i+1)*50), '$' + str(order.getOrderItems()
                      [i].getPrice(order.getPricingStyle())/100), font=normal, fill=(text_color))
            modd.line((500, 760+(i+1)*50, 1400, 760+(i+1)*50),
                      fill=(text_color))
        if resize == True:
            modd.text((520, 720+(i+1)*50), str(order.getOrderItems()
                      [i].getQuantity()), font=normal, fill=(text_color))
            modd.text((650, 720+(i+1)*50), str(order.getOrderItems()
                      [i].getProduct()), font=shmol, fill=(text_color))
            modd.text((1050, 720+(i+1)*50), '$' + str(order.getOrderItems()
                      [i].getUnitPrice(order.getPricingStyle())/100), font=normal, fill=(text_color))
            modd.text((1200, 720+(i+1)*50), '$' + str(order.getOrderItems()
                      [i].getPrice(order.getPricingStyle())/100), font=normal, fill=(text_color))
    ...
    # Total
    modd.text((520, 1600), 'Total: $' + str(order.getOrderTotal()/100),
              font=normal, fill=(text_color))
