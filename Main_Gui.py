...
from Parallel_Processor import forceParallelProcessing, pollForParallelProcessing
...
if __name__ == "__main__":
    ...
    def SyncOrders(app, database):  # sync orders from sheets
        ...
        if Etsy == "True":
            ...
            if not success:
                app.error(
                    "Authentication Error", "Etsy Request Authentication Failed Ingest Aborted"
                )
                return
    ...
    try:
        ...
        # poll and start parallel processing if needed
        processingResults = pollForParallelProcessing(database, app)
