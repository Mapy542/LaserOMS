...
def RebuildProductsFromSheets(app, database):
    ...
    PricingStyles = database.table("Product_Pricing_Styles")  # Get pricing styles table
    ...
    for i in range(len(PricingStylesName)):  # Insert pricing styles into database
        PricingStyles.insert(
            {
                "style_name": PricingStylesName[i],
                "process_status": "UTILIZE",
                "google_sheet_listing": "TRUE",
            }
        )
    ...
    app.info(
        ...
        str(ProductCount) + " Products Imported From Google Sheet",
...
def DownloadListings(app, database):  # Download listings from google sheets
    ...
    url = settings.search(tinydb.where("setting_name") == "Google_Sheet_Link")[0][
        "setting_value"
    ].strip()  # Get google sheet link
    if url == "":
