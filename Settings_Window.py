...
def VerifySettings(database):
    ...
    if not settings.search((tinydb.Query().setting_name == 'Google_Sheet_Link')):
        settings.insert({'setting_name': 'Google_Sheet_Link',
                        'setting_value': '', 'process_status': "UTILIZE"})
        MadeUpdate = True
    if not settings.search((tinydb.Query().setting_name == 'Show_Task_Priority')):
        settings.insert({'setting_name': 'Show_Task_Priority',
                        'setting_value': 'True', 'process_status': "UTILIZE"})
        MadeUpdate = True
    return MadeUpdate
...
def updatevalue(database):
    ...
    settingsdatabase.update({'setting_value': settingsvalue.value}, tinydb.Query(
...
def Settings(main_window, database):
    ...
    window = Window(main_window, title="Settings",
                    width=680, height=600, layout="grid")
    settingstext = Text(window, text="Settings", size=20, grid=[0, 0, 2, 1])
    listview = ListBox(window, items=[], width=400, height=200,
                       scrollbar=True, grid=[0, 1, 4, 5], command=updatemodifier)
    ...
    submitbutton = PushButton(window, text="Update", grid=[
                              3, 6, 1, 1], command=updatevalue, args=[database])

    resetbutton = PushButton(window, text="Reset to Defaults", grid=[
                             0, 7, 1, 1], command=reset, args=[database])
